...
func createHosts(input string) Hosts {
	...
	v := NewParser(testInput, Host{Port: 22})
	return *v
}
...
func TestHostExistsInGroups(t *testing.T) {
	...
	exportedHosts := map[string][]Host{
		"dbs": {
			{Name: "dbhost1", Port: 22},
			{Name: "dbhost2", Port: 22},
		},
		"ungrouped": {{Name: "myhost1", Port: 22}},
		"apps":      {{Name: "my-app-server2", Port: 3000}},
	}

	for group, ehosts := range exportedHosts {
		for _, ehost := range ehosts {
			if hosts, ok := v.Groups[group]; ok { //nolint:nestif // it's a test
				matched := false
...
func TestReadSSHParameters(t *testing.T) {
	expectedHosts := []Host{
		{Name: "sql-host1", Port: 3306, User: "ubuntu", SSHPass: "ubuntu"},
		{Name: "sql-host2", Port: 3306, User: "ubuntu", PrivateKey: "/tmp/some/key"},
	}
	...
	for i, host := range sqlGroup {
		...
		if expectedHosts[i].SSHPass != host.SSHPass {
